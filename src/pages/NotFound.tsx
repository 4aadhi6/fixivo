...
const TesterIcon = () => (
  <div className="relative w-40 h-80 flex items-center justify-center">
    {/* Handle */}
    <motion.div
      animate={{ y: [0, -10, 0], rotate: [10, 15, 5, 10] }}
    ...
    >
      ...
      <motion.div
        ...
        transition={{ duration: 0.5, repeat: Infinity }}
        className="absolute bottom-12 left-1/2 -translate-x-1/2 w-6 h-6 rounded-full border-2 border-red-900"
      />
    </motion.div>
    ...
    <motion.div
      animate={{ x: [-1, 1, -1] }}
      transition={{ duration: 0.1, repeat: Infinity }}
      className="absolute bottom-4 w-4 h-40 bg-gradient-to-b from-gray-400 to-gray-200 rounded-full"
    />
    ...
    <motion.div
      ...
      transition={{ duration: 0.8, repeat: Infinity }}
      className="absolute bottom-0 w-20 h-20 border-4 border-yellow-400 rounded-full"
    />

    <motion.div
      animate={{ scale: [0, 1.8], opacity: [1, 0], rotate: [0, 45] }}
...
export default function NotFound() {
  ...
  return (
    <div
      ...
      className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 text-center relative overflow-hidden"
    >
      {/* ⚡ BACKGROUND GLOW */}
      <motion.div
        ...
        style={{
          background:
            "radial-gradient(circle at bottom, rgba(250,204,21,0.35), transparent 60%)",
        }}
      />
      ...
      >
        ...
        <p className="text-gray-500 max-w-md mx-auto mb-10">
          Short circuit detected. Move your mouse to see live sparks ⚡
        </p>

        <Link
          to="/"
          className="inline-flex items-center gap-2 bg-yellow-400 text-black px-8 py-4 rounded-xl font-bold shadow-lg hover:bg-yellow-500 transition"
        >
          <Home size={18} />
          GO HOME
        </Link>
