...
export default function WorkerDashboard() {
  ...
  const handleAcceptJob = async (jobId: string) => {
    ...
    try {
      ...
      // 1. Update status via Backend
      await axios.post(
        "https://fixivobeckend.onrender.com/api/bookings/assign",
        {
          bookingId: jobId,
          workerId: user!.uid,
        },
  ...
  const handleCompleteJob = async (job: any) => {
    ...
    try {
      // Use the backend API to verify OTP
      await axios.post(
        "https://fixivobeckend.onrender.com/api/bookings/complete",
        {
          bookingId: job.id,
          otpCode: otp,
          billDetails: {
      ...
      <div className="max-w-5xl mx-auto px-6 -mt-10 grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-900 p-8 rounded-[2.5rem] shadow-sm border border-yellow-100 dark:border-yellow-900/30 overflow-hidden relative">
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-2xl">
                ...
                <p className="text-xl font-black text-green-600 dark:text-green-400">
                  ...
                  {finishedJobs.filter((j) => {
                    ...
                    const d = j.completedAt?.seconds
                      ? new Date(j.completedAt.seconds * 1000)
            ...
            {/* top section changed thought erroer  */}
            <div className="grid grid-cols-2 gap-4">
              {/* Earned */}
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-2xl">
                <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">
          ...
          <div className="neumorph p-8 rounded-[2rem]">
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-black text-gray-900 dark:text-white flex items-center gap-1">
                  <Star className="w-4 h-4 text-yellow-500 fill-yellow-400" />
                  <span>{workerData?.rating ?? "N/A"}</span>
                </span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-gray-500 dark:text-gray-400 font-medium">
